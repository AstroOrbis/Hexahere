...
object HexahereClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexahereServerConfig.holder.config = HexahereServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexahereServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hexahere.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HexahereServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexahereServerConfig.ServerConfig()
    }
